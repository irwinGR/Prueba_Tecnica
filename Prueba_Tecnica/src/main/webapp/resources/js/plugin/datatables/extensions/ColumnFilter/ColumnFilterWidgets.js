...
(function($) {
	...
	if ($.fn.dataTable.fnVersionCheck('1.10.0')) {
		$.fn.dataTableExt.oApi.fnGetColumnData = function(oSettings, iColumn) {
			return oSettings.oInstance.api().column(iColumn, {
				search : 'applied'
			}).data().sort().unique();
		}
	} else {
		/*
		 * Function: fnGetColumnData Purpose: Return an array of table values from a particular column. Returns: array string: 1d data array Inputs:
		 * object:oSettings - dataTable settings object. This is always the last argument past to the function int:iColumn - the id of the column to
		 * extract the data from bool:bUnique - optional - if set to false duplicated values are not filtered out bool:bFiltered - optional - if set
		 * to false all the table data is used (not only the filtered) bool:bIgnoreEmpty - optional - if set to false empty values are not filtered
		 * from the result array Author: Benedikt Forchhammer <b.forchhammer /AT\ mind2.de>
		 */
	 ...
	 * @constructor
	 * @param {object}
	 *            oDataTableSettings Settings for the target table.
	 */
	var ColumnFilterWidgets = function(oDataTableSettings) {
		...
		if ('oColumnFilterWidgets' in oDataTableSettings.oInit) {
			if ('hidden' in oDataTableSettings.oInit.oColumnFilterWidgets) {
		...
		$.each(oDataTableSettings.aoColumns, function(i, oColumn) {
			...
			var t = $(oColumn).attr('className') ? $(oColumn).attr('className').toLowerCase().includes('hidden') : false;
			if (sExcludeList.indexOf('|' + i + '|') < 0 && !t) {
	 ...
	 * @constructor
	 * @param {object}
	 *            $Container The jQuery object that should contain the widget.
	 * @param {object}
	 *            oSettings The target table's settings.
	 * @param {number}
	 *            i The numeric index of the target table column.
	 * @param {object}
	 *            widgets The ColumnFilterWidgets instance the widget is a member of.
	 */
	var ColumnFilterWidget = function($Container, oDataTableSettings, i, widgets) {
		...
		// JALZ se agrega clase para inicializar select2
		widget.$Select = $('<select></select>').addClass('widget-' + widget.iColumn).addClass('ColumnFilterWidgetSelect2').change(
				function() {
					...
					// JALZ se agrega estilo select2-selection__choice y select2-selection__choice_no_float para que no flote a la izquierda
					$TermLink = $('<a class="select2-selection__choice_no_float select2-selection__choice filter-term" href="#"></a>').addClass(
							'filter-term-' + sText.toLowerCase().replace(/\W/g, '')).text(sText).click(function() {
					...
					if (widgets.$TermContainer) {
						...
						// JALZ agregarlos despues del select2
						widget.$Container.append($TermLink);
					}
	...
	ColumnFilterWidget.prototype.fnSortOptions = function() {
		var widget = this, $options = widget.$Select.find('option').slice(1);
	...
	/**
	 * On each table draw, update filter menu items as needed. This allows any process to update the table's column visiblity and menus will still be
	 * accurate.
	 * 
	 * @method fnDraw
	 */
